...
//      TR::ILOpCodes loadOp = comp->il.opCodeForIndirectLoad(TR_SInt32);
      ...
      if (!isNonZero)
         ...
         if (addOrSubNode->getFirstChild()->getOpCode().isShift() && addOrSubNode->getFirstChild()->getRegister())
            ...
            generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
            generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpOpCode(), node, r, static_cast<int32_t>(0), TR::InstOpCode::COND_BE, skipAdd);
            }
         else
            {
            generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
            generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpOpCode(), node, targetRegister, static_cast<int32_t>(0), TR::InstOpCode::COND_BE, skipAdd);
            }
      ...
      //case A/(+-)2^x - division is done using shifts
         ...
         if (isDivision)
            ...
            if (!isUnsigned && !firstChild->isNonNegative())
               ...
               generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
               cFlowRegionStart->setStartInternalControlFlow();
               generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CG, node, firstRegister, static_cast<int32_t>(0),TR::InstOpCode::COND_BNL, skipSet);
...
iDivRemGenericEvaluator(TR::Node * node, TR::CodeGenerator * cg, bool isDivision, TR::MemoryReference * divchkDivisorMR)
   ...
   if (needCheck)
      ...
      generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::C, node, sourceRegister, static_cast<int32_t>(-1), TR::InstOpCode::COND_BNE, doDiv);
      cursor =
...
OMR::Z::TreeEvaluator::lmulhEvaluator(TR::Node * node, TR::CodeGenerator * cg)
   ...
   if (secondChild->getOpCodeValue() == TR::lconst)
      ...
      generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
      cFlowRegionStart->setStartInternalControlFlow();
      generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CG, node, sourceRegister, static_cast<int32_t>(0), TR::InstOpCode::COND_BNL, posMulh);
